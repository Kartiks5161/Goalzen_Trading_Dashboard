...
# Optional ML libraries (safe if not installed)
try:
...
try:
    from nsepython import nse_fetch  # used only in fallback
except Exception:
    nse_fetch = None
...
def tune_threshold_cv(best_estimator, X_train, y_train, n_splits=3):
    ...
    for tr, va in tscv.split(X_train):
        m = Pipeline(best_estimator.steps)
...
with tab_overview:
    ...
    if have_ohlc:
        fig.add_trace(go.Candlestick(x=df_view.index,
                                     ...
                                     decreasing_line_color=\"#DC2626\", name=\"Price\"), row=1, col=1)
    else:
    ...
    if (\"best_est\" not in st.session_state) or (st.session_state.get(\"trained_model_name\") != model_choice):
        with st.status(\"Training model…\", expanded=False) as s:
            search = RandomizedSearchCV(
                pipe, param_distributions=grids[model_choice],
...
with tab_predict:
    ...
    if missing:
        ...
        if abs(confidence - th) < 0.05:
            st.warning(\"Confidence is close to the decision threshold. Treat as **low conviction**; consider waiting.\")
...
with tab_insights:
    ...
    if importances is None:
        ...
        r = permutation_importance(best_est, X_test[:n], y_test[:n], n_repeats=8, random_state=GLOBAL_SEED, n_jobs=1)
